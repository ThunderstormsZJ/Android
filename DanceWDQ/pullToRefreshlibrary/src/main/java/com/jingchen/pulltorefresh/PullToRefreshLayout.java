...
public class PullToRefreshLayout extends RelativeLayout {
    ...
    public View getPullableView() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if(v instanceof RecyclerView){
                ((RecyclerView)v).setItemAnimator(new RecyclerItemAnimator());
            }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (emptyView != null) {
            int emptyWidth = emptyView.getMeasuredWidth();
            int emptyHeight = emptyView.getMeasuredHeight();
            emptyView.layout((refreshView.getMeasuredWidth() - emptyWidth) / 2,
                    (refreshView.getMeasuredHeight() - emptyHeight) / 2,
                    (refreshView.getMeasuredWidth() - emptyWidth) / 2 + emptyWidth,
                    (refreshView.getMeasuredHeight() - emptyHeight) / 2 + emptyHeight);
        }
    }
