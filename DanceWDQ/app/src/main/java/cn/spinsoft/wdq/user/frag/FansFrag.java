 ...
 */
public class FansFrag extends BaseFragment implements RecyclerItemClickListener, Handler.Callback,
        PullToRefreshLayout.OnPullListener {
    private static final String TAG = FansFrag.class.getSimpleName();
    ...
    protected void initViewAndListener(View root, Bundle savedInstanceState) {
        ...
        emptyView.setEmptyReason("还没有粉丝");
        mPtrl.setEmptyView(emptyView);
        RecyclerView mFansRv = (RecyclerView) mPtrl.getPullableView();
        mFansRv.setLayoutManager(new LinearLayoutManager(root.getContext(), LinearLayoutManager.VERTICAL, false));
        mAdapter = new UserItemAdapter(null, this);
        mFansRv.setAdapter(mAdapter);
    }

    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        super.onActivityCreated(savedInstanceState);
        mHandler.addCallback(UserHandler.CHILD_FANS, this);
    }
...
//            getActivity().finish();
    ...
    public boolean handleMessage(Message msg) {
        if (msg.what == R.id.msg_user_fans_got) {
            if (msg.obj != null) {
                DancerListWithInfo listWithInfo = (DancerListWithInfo) msg.obj;
                if (UserHandler.Status.pageIdx_fans == 1) {
    ...
    public void onRefresh(PullToRefreshLayout pullToRefreshLayout) {
        UserHandler.Status.pageIdx_fans = 1;
        mHandler.sendEmptyMessage(R.id.msg_user_get_fans);
    }

    @Override
    public void onLoadMore(PullToRefreshLayout pullToRefreshLayout) {
        UserHandler.Status.pageIdx_fans++;
        mHandler.sendEmptyMessage(R.id.msg_user_get_fans);
    }
