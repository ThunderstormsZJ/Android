...
public final class NimUIKit {
     ...
     * @param sessionType   会话类型
     * @param customization 定制化信息。针对不同的聊天对象，可提供不同的定制化。
     * @param nickName      初始化用户名称
     */
    public static void startChatting(Context context, String id, SessionTypeEnum sessionType, SessionCustomization customization, String nickName) {
        if (sessionType == SessionTypeEnum.P2P) {
            P2PMessageActivity.start(context, id, customization, nickName);
        } else if (sessionType == SessionTypeEnum.Team) {
            TeamMessageActivity.start(context, id, customization, null);
        }
    }
